...
import { mockMaintenanceLogs, mockPlants } from "@/data/mockData";
import { Plus, Search, Filter, Trash2, X, Wrench, Calendar, ClipboardCheck } from "lucide-react";
...
export default function MaintenancePage() {
  ...
  const handleLogActivity = (e: React.FormEvent) => {
    e.preventDefault();
    const plant = mockPlants.find((p) => p.plant_id === form.plant_id);
    const newLog = {
      log_id: `ML-${String(logs.length + 1).padStart(4, "0")}`,
      activity_type: form.activity_type.trim() || "General Care",
      date: form.date,
      note: form.note.trim() || "Routine care performed.",
  ...
  const handleDeleteLog = (id: string) => {
    setLogs(logs.filter((l) => l.log_id !== id));
  };

  const filteredLogs = logs.filter((log) => {
    const matchesSearch =
      log.plant_name.toLowerCase().includes(search.toLowerCase()) ||
    ...
    const matchesActivity = activityFilter === "all" ? true : log.activity_type === activityFilter;
  ...
  return (
    <DashboardLayout>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-7">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Maintenance &amp; Care Logs</h1>
          <p className="text-gray-500 mt-1 text-sm">Oversee routine botanical care logs, pruning, repotting, and soil upkeep activities.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Total Logs</p>
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Plants Maintained</p>
            <p className="text-3xl font-bold text-blue-600 mt-1">
              {new Set(logs.map((l) => l.plant_id)).size}
            </p>
          </div>
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm font-bold text-gray-800 mt-1 truncate max-w-[170px]">{logs[0]?.activity_type || "None"}</p>
            <p className="text-xs text-gray-400">{logs[0]?.date || "—"}</p>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-wrap gap-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-gray-800">Activity Log History</h2>
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-1.5">
              ...
              >
                ...
                {Array.from(new Set(logs.map((l) => l.activity_type))).map((t) => (
          ...
          <table>
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                {["Log ID", "Date", "Activity Type", "Plant Specimen", "Observations & Notes", "Actions"].map((h) => (
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredLogs.map((log) => (
                <tr key={log.log_id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-5 py-4 text-xs font-mono text-gray-500 font-medium">{log.log_id}</td>
                  <td className="px-5 py-4 text-xs text-gray-600 whitespace-nowrap">{log.date}</td>
                  <td className="px-5 py-4">
                    <span className={`badge ${activityColors[log.activity_type] ?? "status-clear"}`}>
                      {log.activity_type}
                    </span>
                  </td>
                  <td className="px-5 py-4">
                    <p className="text-sm font-semibold text-gray-800">{log.plant_name}</p>
                    <p className="text-xs font-mono text-gray-400">{log.plant_id}</p>
                  </td>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
              <div>
                <h2 className="text-lg font-bold text-gray-900">Log New Activity</h2>
                <p className="text-xs text-gray-500 mt-0.5">Record a maintenance or care event for a plant</p>
              </div>
            ...
            <form onSubmit={handleLogActivity} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Plant Specimen (plant_id)</label>
                <select
                  value={form.plant_id}
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Activity Type</label>
                <input
                  type="text"
                  placeholder="e.g. Pruning, Repotting, Cleaning, Soil Treatment..."
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Observations &amp; Note</label>
                <textarea
                  rows={3}
              ...
              <div className="pt-2 border-t border-gray-100 flex gap-3">
                ...
                <button type="submit" className="flex-1 py-2.5 bg-[#1B3B2C] hover:bg-[#14532d] text-white rounded-xl text-sm font-semibold transition shadow-sm">Save Log</button>
              </div>
            </form>
          </div>
        </div>
